...
- **The enrichment cache signs on *inputs*, not on enrichment *code* — so code changes are silently ignored.** `get_cached_df` / `write_cached_df` in [openavmkit/utilities/cache.py](openavmkit/utilities/cache.py) key each `cache/<name>.cols.parquet` with `_get_df_signature(df_in, <settings>)` — the base DataFrame's shape plus that enrichment's settings dict. When you change an enrichment's *code* while the base df and its settings block are unchanged (e.g. add a variable to `CensusService.get_census_map`, or add a tag in `OpenStreetMapService._get_tags`), the signature still matches: the stale cached output is returned, the fetch is skipped, and the only sign is a `--> found cached data` log (`_enrich_df_census` returns early at [openavmkit/data.py](openavmkit/data.py) line ~1221). Unlike the *settings*-add-columns case above, **this raises no error at all.** Fix: manually delete the affected `cache/<name>.cols.parquet` + `.signature.json` (and any raw-pull artifact like `cache/osm/water.parquet`). Don't rely on `clear_cache()` — its `.cols`/`.rows` paths are malformed (double-dot, ~line 160) so it won't remove them.
- **Ratio study and VEI measure ratios against *different* denominators.** `ratio_study.py` uses the raw `sale_price` column as ground truth (it never reads `sale_price_time_adj`; `time_adjustment.py` writes that as a *separate* column and never overwrites `sale_price`). But `get_vertical_equity_scores` in [openavmkit/vertical_equity_study.py](openavmkit/vertical_equity_study.py) (called from [openavmkit/modeling.py](openavmkit/modeling.py) line ~1410) divides by the model's `dep_var`, which for a time-adjusted target is **not** raw sale price. For a model trained on a time-adjusted target, the ratio-study median-ratio tables and the VEI column are therefore computed against different reference prices and will not reconcile — and a well-calibrated model can show a median ratio ≠ 1.0 purely from the time-adjustment gap. Don't read a sub-1.0 ratio-study median as pure under-prediction.
